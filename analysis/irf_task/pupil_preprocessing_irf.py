...
class pupilPreprocessSession(object):
    ...
    def trial_params(self):
        ...
        for t in range(self.nr_trials):
            try:
                blinks_nr[t] = sum((self.blink_start_times > self.sound_times[t]) * (self.blink_start_times < self.response_times[t]+1000))
            except:
                shell()
        ...
        for t in range(self.nr_trials):
            saccades_in_trial_indices = (self.saccade_start_times > self.sound_times[t] - 500) * (self.saccade_start_times < self.response_times[t] + 1500)
    ...
    def process_runs(self, alias, artifact_rejection='strict', create_pupil_BOLD_regressor=False):
        ...
        ## CHANGE HERE
        self.sound_times = np.array(self.phase_times['trial_phase_EL_timestamp'][(self.phase_times['trial_phase_index'] == 1)])
        self.response_times = np.array(self.phase_times['trial_phase_EL_timestamp'][(self.phase_times['trial_phase_index'] == 2)])
        #################
    ...
    def process_across_runs(self, aliases, create_pupil_BOLD_regressor=False):
        # scalars:
        # pupil_b is pupil baseline before sound/cue
        # pupil_d is pupil dilation locked to response
        # pupil_cd is pupil dilation locked to sound/cue
        
        # load data:
        parameters = []
        pupil_BOLD_regressors = []
        bp_bp = [] # baseline pupil
        tpr_bp = [] # -0.5-2.5 secs # locked to choice
        tpr_cd_bp = [] # 1-2.5 secs # locked to sound/cue
        RT = []
        for alias in aliases:
            ...
            sound_times = np.array(phase_times['trial_phase_EL_timestamp'][(phase_times['trial_phase_index'] == 1)]) - session_start
            response_times = np.array(phase_times['trial_phase_EL_timestamp'][(phase_times['trial_phase_index'] == 2)]) - session_start
            rt = response_times - sound_times
            RT.append(np.array(rt))
                        
            # baseline pupil measures:
            bp_bp.append( np.array([np.mean(pupil_bp[(time>i-500)*(time<i)]) for i in sound_times]) )
            # phasic pupil responses 
                # response interval [0, 0.5] because very fast reaction times
            tpr_bp.append( np.array([np.mean(pupil_bp[(time>i)*(time<i+500)]) for i in response_times]) - bp_bp[-1]  )
                #  cue interval [0.25,1]
            tpr_cd_bp.append( np.array([np.mean(pupil_bp[(time>i+250)*(time<i+1000)]) for i in sound_times]) - bp_bp[-1]  )

        # join over runs:
        parameters_joined = pd.concat(parameters)

        bp_bp = np.concatenate(bp_bp) # baseline pupil
        tpr_bp = np.concatenate(tpr_bp)        
        tpr_cd_bp = np.concatenate(tpr_cd_bp)
        RT = np.concatenate(RT)
        
        # add to dataframe and save to hdf5:
        parameters_joined['pupil_b'] = bp_bp
        parameters_joined['pupil_d'] = tpr_bp
        parameters_joined['pupil_cd'] = tpr_cd_bp
        parameters_joined['RT'] = RT / 1000 # it's in milliseconds
